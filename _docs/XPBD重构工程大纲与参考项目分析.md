...
- fixed-step (scheduled) + projection + velocity update 的 step 结构
...
native solver 的 `step_scene(handle, dt, simulation_frequency, max_simulation_steps_per_frame)` 应明确包含：

- transform input update
- external force
- fixed-step loop (scheduled from accumulated time)
- collision update
- position projection
- output extraction
