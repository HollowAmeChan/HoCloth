...
- Ported the Point path of `ColliderCollisionConstraint`: MC2-style `ColliderCollisionMode`, per-step point collider loop, particle AABB filtering, Sphere/Plane/Capsule detection helpers, friction/collision-normal writeback, and BoneSpring velocity/max-distance handling are now in C++. Edge collider collision and aggregation buffers remain pending.
