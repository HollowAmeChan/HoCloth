...
| `VirtualMesh/VirtualMesh.cs` | partial | `virtual_mesh/virtual_mesh.*`; core arrays, fixed list/AABB, bind pose, transform restore rotations, vertex-to-triangle and vertex-to-vertex proxy adjacency arrays, baseline arrays, parent-driven baseline/root/depth/local-pose builder, deterministic Dispose reset, optimization entry, and reduction organization entry points are present |
...
| `VirtualMesh/Function/VirtualMeshProxy.cs` | partial | `virtual_mesh/virtual_mesh.*`; fixed-list/AABB, vertex bind pose, vertex-to-transform rotation, vertex-to-triangle lists, vertex-to-vertex compressed adjacency storage, Mesh edge baseline parent generation, Bone transform baseline generation, runtime proxy baseline generation, and runtime proxy skinning/writeback entry points are present; full proxy conversion/edge flag/reduction/custom skinning remain |
...
| `VirtualMesh/Function/VirtualMeshSerialization.cs` | partial | `virtual_mesh/virtual_mesh_serialization.*`; share/unique serialization data containers, structured simple-array restore, raw-byte proxy array restore including `FixedList32Bytes<uint>` vertex-to-triangle decode and vertex-to-vertex compressed adjacency arrays, unique transform-record restore, center fixed list, and baseline fallback are present; packed hash dictionaries remain deferred |
