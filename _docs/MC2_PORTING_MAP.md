...
| VirtualMesh | partial | `_native/include/hocloth/virtual_mesh/`; structured share/unique PreBuild deserialization, raw-byte proxy array restoration, edge-to-triangle dictionary restore, container ownership, MC2 `joinIndices`, AddMesh/center-space helpers, and reduction transform remap storage are now present |
...
| `VirtualMesh/VirtualMesh.cs` | partial | MC2 这里主要是 `VirtualMesh` 的数据字段、生命周期、Dispose、有效性/计数/调试字符串等 class core。HoCloth 已将 `virtual_mesh/virtual_mesh.cpp` 收敛为 core-only 文件；InputOutput/Proxy/Reduction 行为已移入对应拆分文件，不要再把这些行为误判为 MC2 主类职责 |
...
| `VirtualMesh/Function/VirtualMeshInputOutput.cs` | partial | Target split: `virtual_mesh/virtual_mesh_input_output.cpp` plus `manager/render/render_setup_data_serialization.*`. MC2 ownership includes `ImportFrom(RenderSetupData)`, `ImportMeshType`, `ImportBoneType`, `BuildBoneConnection`, Mesh/Bone import, `SelectionMesh`, `CalcSelectionMergin`, `AddMesh`, `SetTransform`/`SetCenterTransform`/`SetSkinRoot`, `SetCustomSkinningBones`, `CompareSpace`, `CenterTransformTo`, and Unity-only `ExportToMesh`. Pure data conversion and space helpers are now concentrated here; Unity Mesh/Renderer acquisition remains a Blender transfer boundary |
| `VirtualMesh/Function/VirtualMeshMapping.cs` | partial | `virtual_mesh/virtual_mesh_mapping.cpp`; MC2 ownership is `Mapping(...)`, direct/search mapping work data, mapping weight calculation, proxy-neighbor weight generation, and mapping mesh type transition. `CompareSpace()` and `CenterTransformTo()` now live in `virtual_mesh_input_output.cpp` to match MC2 ownership. Runtime render-mapping writeback integration and final MeshCloth pipeline parity remain |
| `VirtualMesh/Function/VirtualMeshOptimization.cs` | complete | `virtual_mesh/virtual_mesh_optimization.cpp`; MC2 duplicate-triangle/near-planar quad pair removal path is present |
| `VirtualMesh/Function/VirtualMeshProxy.cs` | partial | `virtual_mesh/virtual_mesh_proxy.cpp`. MC2 ownership includes `ConvertProxyMesh`, `ProxyCreateFixedListAndAABB`, vertex bind pose, vertex-to-transform rotation, vertex-to-triangle lists, triangle direction optimization, triangle-derived normal/binormal feed, vertex-to-vertex compressed adjacency, edge-to-triangle cache, edge cut flags, invalid-to-fixed conversion, `ApplySelectionAttribute`, Mesh/Transform baseline generation, `CreateBaseLinePose`, `CreateVertexRootAndDepth`, `ProxyNormalAdjustment`, `CreateCustomSkinning`, and `CreateAngleCalcLocalRotation`. Current HoCloth implementation now lives in this split file; custom-skinning-bone registration (`SetCustomSkinningBones`) stays InputOutput, while custom-skinning weight generation stays Proxy |
| `VirtualMesh/Function/VirtualMeshReduction.cs` | partial | `virtual_mesh/virtual_mesh_reduction.cpp` plus `reduction/*.hpp`. MC2 ownership includes `Reduction`, `InitReductionWorkData`, Same/Simple/Shape algorithm chain, `Organization*`, remap/basic-data copy, line/triangle rebuild, transform/skin-bone index remap, MC2 `joinIndices` store-back, and post-store derived topology refresh. Current HoCloth implementation now lives in this split file; full parity audit remains |
| `VirtualMesh/Function/VirtualMeshSerialization.cs` | complete | `virtual_mesh/virtual_mesh_serialization.*`; share/unique serialization data containers, structured simple-array restore, raw-byte proxy array restore including `FixedList32Bytes<uint>` vertex-to-triangle decode, vertex-to-vertex compressed adjacency arrays, edge-to-triangle key/value restoration, custom skinning indices, center pose fields, unique transform-record restore, center fixed list, and baseline fallback are present |
| `VirtualMesh/Function/VirtualMeshWork.cs` | complete | `virtual_mesh/virtual_mesh_work.cpp`; average/max vertex distance sampling, vertex-index GridMap creation, center-transform ray conversion, and triangle/edge ray intersection are present |

### VirtualMesh Native Split Rule

MC2 的 `VirtualMesh` 是 C# partial class，但职责边界以 `Function/*.cs` 为准。HoCloth C++ 文件也按这个边界拆，而不是按“哪个函数调用了谁”拆：

- `virtual_mesh.cpp`: only `VirtualMesh.cs` class core, storage lifecycle, Dispose, validity/count/debug query. Do not add Function-layer behavior back into this file.
- `virtual_mesh_input_output.cpp`: `VirtualMeshInputOutput.cs`; import/export boundary, Mesh/Bone import, BoneConnection construction, transform records, space conversion, AddMesh, selection mesh feed, custom-skinning bone registration.
- `virtual_mesh_proxy.cpp`: `VirtualMeshProxy.cs`; proxy conversion, topology caches, normals/tangents, baseline, vertex root/depth, selection attributes, custom-skinning weights.
- `virtual_mesh_reduction.cpp`: `VirtualMeshReduction.cs`; reduction orchestration and organization/store-back.
- Existing split files keep their MC2 file ownership: `virtual_mesh_mapping.cpp`, `virtual_mesh_optimization.cpp`, `virtual_mesh_serialization.cpp`, `virtual_mesh_work.cpp`.
